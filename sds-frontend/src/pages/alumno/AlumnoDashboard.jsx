...
const PaymentSummary = ({ alumnoActivo }) => {
    const { data: payments = [], isLoading, isError } = useQuery({
        queryKey: ['pagos', 'mis-pagos', alumnoActivo?.id],
        queryFn: async () => (await pagosAPI.getMisPagos(alumnoActivo.id)).data.data || [],
        enabled: !!alumnoActivo,
        staleTime: 60 * 1000,
    });
...
const LiveBanner = ({ alumnoId }) => {
    const { data } = useQuery({
        queryKey: ['transmision-en-vivo', alumnoId],
        queryFn: async () => (await transmisionesAPI.enVivo(alumnoId)).data,
        ...
        <div className="max-w-4xl mx-auto pb-12 animate-fade-in">
            ...
            <div className="mb-8 text-center sm:text-left">
                <h1 className="text-3xl font-bold text-gray-900 tracking-tight">
                    Hola, {alumnoActivo?.nombre || user?.nombre} 👋
                </h1>
            ...
            <LiveBanner alumnoId={alumnoActivo?.id} />

            {/* Resumen financiero familiar */}
            <PaymentSummary alumnoActivo={alumnoActivo} />

...
router.post('/recordatorios-masivos', async (req, res) => {
    try {
        // Obtener parámetros opcionales de mes y año
        const { mes, anio } = req.body;

        // Si se proporcionan mes y año, filtrar por ellos; sino, obtener todos los pendientes
        let pagosPendientes;
        if (mes && anio) {
            pagosPendientes = await PagosModel.findPendientesByMonth(parseInt(mes), parseInt(anio));
        } else {
            pagosPendientes = await PagosModel.findPendientes();
        }
        ...
        const mensajePeriodo = mes && anio ? ` (${mes}/${anio})` : '';
        res.json({
            success: true,
            message: `Proceso finalizado${mensajePeriodo}. Enviados: ${enviados}, Errores: ${errores}`,
...
router.post('/notificar-evento-masivo/:eventoId', async (req, res) => {
    try {
        ...
        for (const inscrito of evento.inscritos) {
            try {
                // inscrito ya contiene nombre, apellido, email y telefono del alumno
                // (viene del JOIN con usuarios en EventosModel.findById)
                const emailDestino = inscrito.email || inscrito.email_padre;
                ...
                console.error(`Error enviando a inscrito ${inscrito.inscripcion_id}:`, error);
                errores++;
            }
        }

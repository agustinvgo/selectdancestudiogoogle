const { NodeSSH } = require('node-ssh');
require('dotenv').config();

const ssh = new NodeSSH();

async function checkRemoteStatus() {
    try {
        await ssh.connect({
            host: process.env.SSH_HOST || '187.77.62.115',
            username: process.env.SSH_USER || 'root',
            password: process.env.SSH_PASSWORD || 'YERkw147?N@C)?(CeW1#'
        });

        console.log('--- PM2 LIST ---');
        const list = await ssh.execCommand('pm2 list');
        console.log(list.stdout);

        console.log('--- DIR UTILS ---');
        const dir = await ssh.execCommand('ls -la /var/www/sds-backend/src/utils');
        console.log(dir.stdout);

        console.log('--- LAST ERRORS ---');
        const errorLogs = await ssh.execCommand('tail -n 100 /root/.pm2/logs/sds-backend-error.log');
        console.log(errorLogs.stdout);

        ssh.dispose();
    } catch (err) {
        console.error(err);
    }
}

checkRemoteStatus();
